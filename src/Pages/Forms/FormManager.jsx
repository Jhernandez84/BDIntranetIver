...
export const FormManager = () => {
    ...
    const DBEvento = 'L&R Sin Limites'

    const camposContacto = { Rut: '', Nombres: '', ApellidoPaterno: '', ApellidoMaterno: '', FechaNacimiento: '', NumeroContacto: '', DV: '', email: '', Direccion: '', EventoEstado: 'Inscrito', esStaff: 'No', staff: 'Participante', iglesiaVisita: 'IverChile', encabezado: 'Levántate y Resplandece', lema: 'Sin límites', invitacion: 'Por favor reserva tu lugar ingresando tus datos aquí' }
        ...
        collectionRef2
            ...
            .then((querySnapshot) => {
                if (!querySnapshot.empty) {
                    ...
                    swal("Ya está Inscrito", "Puede estar tranquil@, ya está inscrit@ para este evento", "success")
                    setData([])
                    setContacto(camposContacto)
                } else {
                    SetShowForm(true)
    ...
    useEffect(() => {
        contacto.Rut.length === 8 || (contacto.Rut.length === 7 & parseInt(contacto.Rut.charAt(0)) >= 3) ? (
            // setContacto(camposContacto),
            getDataByRut()
            // console.log(contacto)
        ) : (
    ...
    const SendData = async (e) => {
        ...
        setData([]),
            SetShowForm(false),
            setContacto(camposContacto)
        swal(`Inscripción exitosa`, `${contacto.Nombres} estaremos esperándote `, "success")
        setContacto(camposContacto)
    }
                    ...
                    <form>
                        <div className="mb-4 text-center" >
                            ...
                            <div className="form-input">
                                ...
                                <input name="Rut" maxLength={"8"} onChange={getFormValues} value={contacto.Rut} type="text" className="form-control" id="inputContactName" aria-describedby="inputName" required></input>
                                <p><i>antes del dígito verificador: 12345678</i></p>
                            </div>
                            {showForm === true ? (
                                <>
                                    <div className="form-input">
                                        <label htmlFor="Nombres" className="form-label">Nombre Completo</label>
                                        <input name="Nombres" onChange={getFormValues} value={contacto.Nombres} type="text" className="form-control" id="inputContactName" aria-describedby="inputName" required></input>
                                    </div>
                                    <div className="form-input">
                                        <label htmlFor="ApellidoPaterno" className="form-label">Apellidos</label>
                                        <input name="ApellidoPaterno" onChange={getFormValues} value={contacto.ApellidoPaterno} type="text" className="form-control" id="inputContactName" aria-describedby="inputName" required></input>
                                    </div>
                                    <div className="form-input">
                                        <label htmlFor="NumeroContacto" className="form-label">Teléfono de contacto</label>
                                        <input name="NumeroContacto" onChange={getFormValues} value={contacto.NumeroContacto} type="phone" className="form-control" id="inputContactPhone" aria-describedby="inputPhone"></input>
                                    </div>
                                    <div className="form-input">
                                        <label htmlFor="FechaNacimiento" className="form-label">Fecha de nacimiento</label>
                                        <input name="FechaNacimiento" onChange={getFormValues} value={contacto.FechaNacimiento} type="date" className="form-control" id="inputContactMail" aria-describedby="inputMail"></input>
                                    </div>
                                    <div className="form-input">
                                    ...
                                    <div className="d-grid gap-2 col-6 mx-auto">
                                        {contacto.Rut && contacto.FechaNacimiento && contacto.Nombres && contacto.NumeroContacto && contacto.iglesiaVisita ? (
                                            <>
                                                <button type="submit" onClick={SendData} className="btn btn-success btn-registrarme">
                                                    Enviar
                                                </button>
                                            </>
                                        ) : ([])}
